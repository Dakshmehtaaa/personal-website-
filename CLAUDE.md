...
- **For-companies resource marks are sized one by one** with `style="--mw:NNpx"` on each `img`, so a roundel and a long wordmark read at the same weight: start from `h = 54 × aspect^-0.42` (capped at 112px wide), then nudge thin marks up and dense or solid ones down by eye. A new resource needs its own `--mw`. Logos drawn on their own disc or panel (GRI, ESRS, EU Taxonomy) take `sys-res-mark--solid` so dark mode doesn't invert them into white blobs; no glow or halo on hover anywhere.
